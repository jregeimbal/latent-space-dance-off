...
class LeaderboardEntry(BaseModel):
    ...
    def from_model_score(cls, model_score, rank, svg_files=None):
        return cls(
            ...
            creativity_score=model_score.creativity_score,
            aesthetics_score=model_score.aesthetics_score,
            complexity_score=model_score.complexity_score,
            total_score=model_score.total_score,
            judgment_count=model_score.judgment_count,
...
class RankingSystem:
    ...
    def _ensure_output_dirs(self):
        (Path(self.config.OUTPUT_DIR) / self.config.leaderboards_dir).mkdir(parents=True, exist_ok=True)
    ...
    def aggregate_all_judgments(self, run_data, judgments):
        ...
        for judgment in judgments:
            model_name = judgment.svg_id.split("_")[0]
            if model_name not in scores:
                scores[model_name] = ModelScore(model_name=model_name, creativity_score=0.0, 
                                                  aesthetics_score=0.0, complexity_score=0.0,
                                                  total_score=0.0, judgment_count=0)
            
            model_score = scores[model_name]
            if judgment.creativity_score is not None:
                model_score.creativity_score += judgment.creativity_score
            if judgment.aesthetics_score is not None:
                model_score.aesthetics_score += judgment.aesthetics_score
            if judgment.complexity_score is not None:
                model_score.complexity_score += judgment.complexity_score
            model_score.judgment_count += 1
        
        for model_name in scores:
            model_score = scores[model_name]
            if model_score.judgment_count > 0:
                model_score.creativity_score /= model_score.judgment_count
                model_score.aesthetics_score /= model_score.judgment_count
                model_score.complexity_score /= model_score.judgment_count
                model_score.total_score = (
                    model_score.creativity_score * self.config.default_creativity_weight +
                    model_score.aesthetics_score * self.config.default_aesthetics_weight +
                    model_score.complexity_score * self.config.default_complexity_weight
                )
            else:
                model_score.total_score = 0.0
    ...
    def calculate_final_ranking(self, model_scores):
        ...
        for rank, model_score in enumerate(sorted_models, start=1):
            entry = LeaderboardEntry.from_model_score(model_score, rank, [])
    ...
    def save_leaderboard(self, leaderboard):
        (Path(self.config.OUTPUT_DIR) / self.config.leaderboards_dir).mkdir(parents=True, exist_ok=True)
        filepath = Path(self.config.OUTPUT_DIR) / self.config.leaderboards_dir / f"{leaderboard.run_id}-leaderboard.json"
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(leaderboard.model_dump(), f, indent=2)
        return str(filepath)
    
    def load_leaderboard(self, run_id):
        filepath = Path(self.config.OUTPUT_DIR) / self.config.leaderboards_dir / f"{run_id}-leaderboard.json"
    ...
    def export_to_csv(self, leaderboard, filepath=None):
        if filepath is None:
            filepath = Path(self.config.OUTPUT_DIR) / self.config.leaderboards_dir / f"{leaderboard.run_id}.csv"
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            ...
            for entry in leaderboard.rankings:
                row = [str(entry.rank), entry.model_name, 
    ...
    def get_model_stats(self, leaderboard, model_name):
        for entry in leaderboard.rankings:
