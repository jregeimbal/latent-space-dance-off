...
{judge_blocks_html}
        ...
        svg_b64 = base64.b64encode(opt["svg_code"].encode("utf-8")).decode("ascii")
        pass_contents += f'''        <div class="pass-content" data-pass="{opt["pass_number"]}" style="{display}">
            <div class="svg-container">
                <img src="data:image/svg+xml;base64,{svg_b64}" alt="SVG generated by {html.escape(model)} for theme {html.escape(theme)}" style="max-width:100%;height:auto;">
            </div>
    ...
    for r in result.rounds:
        ...
        for svg in r.svg_results:
            if svg.status == "success" and svg.svg_code:
                svg_b64 = base64.b64encode(svg.svg_code.encode("utf-8")).decode("ascii")
                svgs_html += f'<div style="margin:8px 0"><strong>{html.escape(svg.model_name)}</strong><br>' \
                           f'<img src="data:image/svg+xml;base64,{svg_b64}" alt="{html.escape(svg.model_name)}" style="max-width:100%;height:auto;"></div>'
